---
layout: default
title: Home
permalink: /
---

## About
{: #about}

<figure>
  <img src="/assets/img/beaver-dam.jpg" alt="A beaver dam in a mountain stream">
</figure>

I am a 4th-year PhD candidate at **UC Santa Barbara Geography**, where I study all
things rivers. My work spans scales, from the sediment dynamics of a single beaver pond to
multidecadal, global datasets of the world's largest and most dynamic river systems.

My work spans four themes:

1. **[Calibrating the geomorphic clock](#geomorphic-clock)** —
   reading sediment storage and transit times from how the world's rivers migrate.
2. **[Sediment and carbon storage in beaver-mediated stream corridors](#beaver)** —
   how beaver ponds trap sediment and bury carbon across the American West.
3. **[Mapping a changing flood pulse in a UNESCO World Heritage Site](#okavango)** —
   tracking the shifting annual flood of the Okavango Delta, Botswana.
4. **[Flood adaptation in a changing world](#flood-adaptation)** —
   managed retreat and community relocation as flood-mitigation strategies.

Threaded through all four is a single conviction: we must learn to read rivers — to
understand their physical behavior, and to restore their pre-disturbance processes,
mechanics, and morphologies. Managing rivers with this in mind offers a plethora of
co-benefits — environmental, ecological, social, financial, and agricultural — and gives us
a holistic context with which we may guide restoration, management, action, and ourselves.

[Email](mailto:jamesrees@ucsb.edu) ·
[GitHub](https://github.com/huck-rees) ·
[LinkedIn](https://www.linkedin.com/in/huck-rees-12b205157/)

---

## Research
{: #research}

My work asks how rivers and the ecosystems within them store and move water, sediment, and carbon —
across scales from a single beaver pond to entire continental river systems, and across
timescales from a field season to millennia. It falls into four themes.

### 1. Calibrating the geomorphic clock
{: #geomorphic-clock}

<figure>
  <img src="/assets/img/geomorphic-transit-model.png" alt="Conceptual model of sediment storage and transit through a migrating floodplain">
  <figcaption>Sediment entering a corridor (Q<sub>s</sub>) is repeatedly stored and remobilized as the river migrates; total transit time is the sum of many storage events and transit distances.</figcaption>
</figure>

The centerpiece of my dissertation is a remote sensing-based approach to modelling how
sediment is stored and routed through alluvial river corridors. Using multidecadal, global
Landsat records, I measure how the world's largest and most dynamic rivers migrate and
rework their floodplains, and I use those measurements to estimate sediment and carbon
residence times over long timescales. The goal is a "geomorphic clock" — a way to read
transient sediment storage and transit times directly from how a river's planform, lithology,
climate, and human alteration shape its movement. This work is supported by
[RivMapper](https://github.com/huck-rees/RivMapper), my Python toolkit for building water
masks and quantifying migration from satellite imagery.

### 2. Sediment and carbon storage in beaver-mediated stream corridors
{: #beaver}

<figure>
  <img src="/assets/img/beaver-methods.png" alt="Methods for quantifying sediment storage behind beaver dams across timescales">
  <figcaption>Repeat geomorphic surveys, historical air-photo time series, and depth-incremented cores with radiometric dating together constrain sedimentation and aggradation rates across timescales — from year-to-year to floodplain-aggradation — as an incised channel reconnects to its floodplain.</figcaption>
</figure>

<figure class="photo">
  <img src="/assets/img/beaver-poster.jpg" alt="Huck Rees presenting beaver research at AGU">
  <figcaption>Presenting beaver sediment- and carbon-storage research at AGU.</figcaption>
</figure>

Beaver dams turn narrow mountain streams into complexes of ponds and wet meadows that trap
sediment and bury carbon. Using sediment cores, geomorphic surveys, and drone-based mapping,
I quantify sedimentation and carbon-accretion rates behind beaver dams and ask how they vary
across environments in the American West — and what that means for beaver-based restoration
as a tool for wetland recovery, flood attenuation, sediment routing, and carbon storage.

**Related publication:** Rees, Lininger, Landis & Briles (2024), *Assessing controls on
sedimentation rates and sediment organic carbon accretion in beaver ponds*,
*Science of the Total Environment.*
[DOI](https://doi.org/10.1016/j.scitotenv.2024.174951)

### 3. Mapping a changing flood pulse in a UNESCO World Heritage Site
{: #okavango}

<figure>
  <img src="/assets/img/okavango-change.png" alt="Map of change in inundation across the Okavango Delta">
  <figcaption>Change in inundation across the Okavango Delta (post- minus pre-2010–2012 La Niña): teal marks wetting, brown marks drying, exposing migrating channels, shrinking lakes, and shifting flow toward the Selinda Spillway.</figcaption>
</figure>

The Okavango Delta in Botswana — a UNESCO World Heritage Site — is sustained by an annual
flood pulse whose timing and reach shape one of the planet's most incredible wetland ecosystems. I
build multidecadal satellite records of the Delta's inundation to track how that pulse is
changing, and to understand the tectonic, climatic, and ecological controls behind it.
Recent work reconstructs a 41-year monthly inundation record that reveals an abrupt
redistribution of flood waters across the Delta, identifying localized areas of drying and wetting.

### 4. Flood adaptation in a changing world
{: #flood-adaptation}

<figure>
  <img src="/assets/img/valmeyer-managed-retreat.jpg" alt="Before-and-after maps of managed retreat at Valmeyer, Illinois">
  <figcaption>Managed retreat at Valmeyer, IL: structures in the FEMA 100-year floodplain (1989) were relocated to higher ground after catastrophic flooding, with later construction (2018) concentrated outside the floodzone.</figcaption>
</figure>

<figure class="photo">
  <img src="/assets/img/odanah-managed-retreat.jpg" alt="Fieldwork on a street in Odanah, Wisconsin">
  <figcaption>Fieldwork and stakeholder interviews in Odanah, Wisconsin — a community weighing retreat from repeated flooding.</figcaption>
</figure>

Flooding is among the costliest and most widespread natural hazards, and the choices
communities make — protect, accommodate, or retreat — carry long-lasting consequences. I study the
effectiveness of managed retreat and community relocation as flood-mitigation strategies,
from small Midwestern towns on large river floodplains to post-tsunami reconstruction along the northern 
coast of Japan, connecting empirical flood risk modeling to the social and policy dimensions of adaptation.

**Related publications:**

- Pinter & Rees (2021), *Assessing managed flood retreat and community relocation in the
  Midwest USA*, *Natural Hazards.*
  [DOI](https://doi.org/10.1007/s11069-021-04592-1)
- Pinter, Ishiwatari, … Rees, et al. (2019), *Large-scale managed retreat and structural
  protection following the 2011 Japan tsunami*, *Natural Hazards.*
  [DOI](https://doi.org/10.1007/s11069-019-03602-7)

## Projects & Software
{: #projects}

- **[RivMapper](https://github.com/huck-rees/RivMapper)** — a Python codebase for building
  2D water-mask rasters from Landsat imagery, calculating rates of river migration, and
  estimating sediment storage and transit times in floodplains.

---

## Publications
{: #publications}

Author name in **bold**. See my [CV](#cv) for the complete list of presentations, grants,
and service.

### Peer-reviewed articles

<div class="pubs">
  <div class="pub">
    <div class="pub-year">2024</div>
    <div>
      <div class="pub-title"><a href="https://doi.org/10.1016/j.scitotenv.2024.174951">Assessing controls on sedimentation rates and sediment organic carbon accretion in beaver ponds</a></div>
      <div class="pub-meta"><strong>Rees, J.</strong>, Lininger, K.B., Landis, J., &amp; Briles, C. — <em>Science of the Total Environment</em>, 949, 174951.</div>
    </div>
  </div>
  <div class="pub">
    <div class="pub-year">2024</div>
    <div>
      <div class="pub-title">Working with wood in rivers in the Western United States</div>
      <div class="pub-meta">Ockelford, A., Wohl, E., … <strong>Rees, J.</strong>, … Aarnink, J. — <em>River Research and Applications</em>.</div>
    </div>
  </div>
  <div class="pub">
    <div class="pub-year">2021</div>
    <div>
      <div class="pub-title"><a href="https://doi.org/10.1007/s11069-021-04592-1">Assessing managed flood retreat and community relocation in the Midwest USA</a></div>
      <div class="pub-meta">Pinter, N., &amp; <strong>Rees, J.</strong> — <em>Natural Hazards</em>, 107, 497–518.</div>
    </div>
  </div>
  <div class="pub">
    <div class="pub-year">2019</div>
    <div>
      <div class="pub-title"><a href="https://doi.org/10.1007/s11069-019-03602-7">Large-scale managed retreat and structural protection following the 2011 Japan tsunami</a></div>
      <div class="pub-meta">Pinter, N., Ishiwatari, M., Nonoguchi, A., Tanaka, Y., Casagrande, D., Durden, S., &amp; <strong>Rees, J.</strong> — <em>Natural Hazards</em>, 96, 1429–1436.</div>
    </div>
  </div>
</div>

### In review & in preparation

<div class="pubs">
  <div class="pub">
    <div class="pub-year">submitted</div>
    <div>
      <div class="pub-title">Calibrating the geomorphic clock: remote sensing of sediment storage and transit timescales in alluvial river corridors</div>
      <div class="pub-meta"><strong>Rees, H.</strong>, Ganti, V., Greenberg, E., Li, G., &amp; Caylor, K. — <em>Geophysical Research Letters</em>.</div>
    </div>
  </div>
  <div class="pub">
    <div class="pub-year">submitted</div>
    <div>
      <div class="pub-title">Global sediment transport intermittency is set by river planform</div>
      <div class="pub-meta">McLeod, J.S., Ganti, V., Hampson, G.J., Bell, R.E., Slater, L.J., <strong>Rees, H.</strong>, &amp; Whittaker, A.C. — <em>Nature Geoscience</em>.</div>
    </div>
  </div>
  <div class="pub">
    <div class="pub-year">submitted</div>
    <div>
      <div class="pub-title">Early career perspectives on the next generation of deltaic science</div>
      <div class="pub-meta">WinklerPrins, L., Fontenot, K., … <strong>Rees, H.</strong>, et al. — <em>Estuaries and Coasts</em>.</div>
    </div>
  </div>
  <div class="pub">
    <div class="pub-year">in prep</div>
    <div>
      <div class="pub-title">A 41-year monthly inundation record reveals abrupt redistribution of flood waters across the Okavango Delta since 2012</div>
      <div class="pub-meta"><strong>Rees, H.</strong>, Crompton, O., Lourenço, M., WinklerPrins, L., Larsen, L., Caylor, K.K., &amp; Ganti, V. — for <em>Nature Communications</em>.</div>
    </div>
  </div>
  <div class="pub">
    <div class="pub-year">in prep</div>
    <div>
      <div class="pub-title">A validated land cover classification of the Okavango Delta, 2023</div>
      <div class="pub-meta">WinklerPrins, L., Yang, W., Crompton, O., Lourenço, M., Ruer, M., Taylor, R., <strong>Rees, H.</strong>, &amp; Larsen, L. — for <em>Remote Sensing of the Environment</em>.</div>
    </div>
  </div>
</div>

### Other writing

<div class="pubs">
  <div class="pub">
    <div class="pub-year">2019</div>
    <div>
      <div class="pub-title">Literature review of mechanisms and timescales involved in the termination of the African Humid Period</div>
      <div class="pub-meta"><strong>Rees, J.</strong> — <em>Prized Writing, University of California, Davis</em>.</div>
    </div>
  </div>
</div>

### Selected presentations

<div class="pubs">
  <div class="pub">
    <div class="pub-year">Dec 2025</div>
    <div>
      <div class="pub-title">Calibrating the geomorphic clock: remote sensing of sediment storage and transit timescales in alluvial river corridors</div>
      <div class="pub-meta"><strong>Rees, H.</strong>, Ganti, V., Greenberg, E., Li, G., &amp; Caylor, K. — <em>American Geophysical Union Fall Meeting</em>, New Orleans, LA.</div>
    </div>
  </div>
  <div class="pub">
    <div class="pub-year">Oct 2025</div>
    <div>
      <div class="pub-title">How much sediment can a beaver dam store: synthesizing drivers of beaver pond sedimentation</div>
      <div class="pub-meta">Scamardo, J., Lininger, K.B., &amp; <strong>Rees, H.</strong> — <em>7th International Symposium on River Science</em>, Davis, CA.</div>
    </div>
  </div>
  <div class="pub">
    <div class="pub-year">Dec 2024</div>
    <div>
      <div class="pub-title">Natural and anthropogenic controls on floodplain reworking timescales and transient sediment storage in lowland river corridors</div>
      <div class="pub-meta"><strong>Rees, J.</strong>, Ganti, V., &amp; Caylor, K. — <em>American Geophysical Union Fall Meeting</em>, Washington, D.C.</div>
    </div>
  </div>
  <div class="pub">
    <div class="pub-year">Oct 2024</div>
    <div>
      <div class="pub-title">Sedimentation &amp; carbon accretion in beaver ponds &amp; implications for restoration</div>
      <div class="pub-meta">Lininger, K.B., &amp; <strong>Rees, J.</strong> — <em>BeaverCon</em>, Boulder, CO.</div>
    </div>
  </div>
</div>

*A full list of talks and posters is in my [CV](#cv).*

---

## Media Coverage
{: #media}

<ul>
  <li>
    <a href="https://press.uchicago.edu/ucp/books/book/chicago/W/bo161589950.html"><img class="media-icon" src="https://www.google.com/s2/favicons?domain=press.uchicago.edu&amp;sz=64" alt=""> University of Chicago Press</a> —
    Gies, E. (2022). <em>Water Always Wins: Thriving in an Age of Drought and Deluge.</em>
  </li>
  <li>
    <a href="https://www.westword.com/news/university-colorado-divestment-fossil-fuels-climate-change-15592395"><img class="media-icon" src="https://www.google.com/s2/favicons?domain=westword.com&amp;sz=64" alt=""> Westword</a> —
    Cheshire, C. (2022). <em>As CU Hosts Global Climate Summit, Students Call for Fossil Fuel Divestment.</em>
  </li>
  <li>
    <a href="https://www.pbs.org/newshour/show/the-radical-approach-these-communities-have-taken-to-flood-mitigation"><img class="media-icon" src="https://www.google.com/s2/favicons?domain=pbs.org&amp;sz=64" alt=""> PBS NewsHour</a> —
    Wise, C. (2019). <em>The radical approach these communities have taken to flood mitigation.</em>
  </li>
  <li>
    <a href="https://giving.ucdavis.edu/impacts-giving/small-towns-big-flood-waters"><img class="media-icon" src="https://www.google.com/s2/favicons?domain=ucdavis.edu&amp;sz=64" alt=""> UC Davis</a> —
    Kerlin, K., &amp; Proudman, J. (2019). <em>Small Towns, Big Flood Waters.</em>
  </li>
  <li>
    <a href="https://www.npr.org/2018/08/15/632335735/wisconsin-reservation-offers-a-climate-success-story-and-a-warning"><img class="media-icon" src="https://www.google.com/s2/favicons?domain=npr.org&amp;sz=64" alt=""> NPR</a> —
    Hersher, R. (2018). <em>Wisconsin Reservation Offers a Climate Success Story and a Warning.</em>
  </li>
</ul>

---

## CV
{: #cv}

[**Download full CV (PDF)**](/assets/cv/huck-rees-cv.pdf)

James (Huck) Rees — Department of Geography, University of California, Santa Barbara ·
[jamesrees@ucsb.edu](mailto:jamesrees@ucsb.edu)

### Background

PhD candidate in Geography with extensive expertise in geomorphology, hydrology, and remote
sensing, specializing in modelling sediment storage and transport, surface water detection,
mapping river corridor and floodplain evolution, and quantifying sediment and carbon storage
in beaver-mediated river systems. Skilled in Python, R, and geospatial programming, with a
proven record of interdisciplinary collaboration, grant success, and peer-reviewed
publications. Passionate about applying advanced geospatial analytics and field-validated
insights to create practical, scalable tools that support wetland restoration, flood
protection, and carbon storage.

### Education

- **Ph.D. in Geography** — University of California, Santa Barbara *(in progress)*
  Dissertation: *Calibrating the geomorphic clock: a remote sensing-based approach to
  modelling sediment storage and routing in alluvial river corridors.* GPA: 4.0
- **M.A. in Fluvial Geomorphology** — University of Colorado, Boulder (2023)
  Certificate in Hydrologic Sciences. Thesis: *Sediment and Carbon Storage and Aggradation
  Rates within Beaver-mediated River Corridors in the Southern Rocky Mountains, USA.* GPA: 4.0
- **B.A. in Geology** (*summa cum laude*, Phi Beta Kappa) and **B.A. in History**
  (*cum laude*), Minor in Geographic Information Science — University of California, Davis
  (2019). Thesis: *Managed retreat and community relocation as a flood mitigation measure in
  small Midwest communities.* GPA: 3.84

### Professional Experience

- **Graduate Researcher**, Water, Vegetation & Society Lab / Surface Processes Lab —
  UC Santa Barbara, Department of Geography (Sep 2023–present)
- **Summer Intern**, Imaging Spectroscopy Group — NASA Jet Propulsion Laboratory
  (June 2026–present)
- **Graduate Researcher**, Fluvial Ecogeomorphology Lab — University of Colorado, Boulder,
  Department of Geography (July 2021–Aug 2023)
- **Water Resources Coordinator**, Water Specialty Group — HDR Engineering (Aug 2019–July 2021)
- **Research Assistant**, Childs Meadow Restoration Project — UC Davis, Center for Watershed
  Science (June 2019–Aug 2019)
- **Undergraduate Researcher**, Geomorphology and Hazard Research Group — UC Davis, Department
  of Earth and Planetary Sciences (Oct 2017–June 2019)
- **Research Assistant**, Map Collection Room — UC Davis, Shields Library (May 2017–Oct 2018)

### Honors & Awards

- Top Cited Author Award, *River Research and Applications* (2025)
- Sustainability Award, CU Boulder (2023)
- NSF Graduate Research Fellowship, Honorable Mention (2021)
- Highest Honors Distinction, UC Davis Department of Earth and Planetary Sciences (2019)
- Honors Distinction, UC Davis History Department (2019)
- Outstanding Senior, UC Davis Department of Earth and Planetary Sciences (2019)
- Winner, Prized Writing Competition, Scientific and Technical Writing (2019)
- Department Citation, UC Davis History Department (2019)
- Phi Beta Kappa Honors Society (2018)

### Grants

- Benioff Ocean Sciences Laboratory Research Grant — $6,000 (2025)
- Schmidt Family Foundation Research Accelerator Fellowship — $8,000 (2024)
- Graduate Research Fellowship, Rocky Mountain Biological Lab — $500 (2022)
- Adam Kolff Memorial Research Scholarship, CU Boulder — $5,000 (2022)
- Jeanne DeSana Graduate Research Scholarship, CU Boulder — $5,000 (2022)
- Fulbright Open Study/Research Grant, South Pacific Island Nations — ~$20,000 (2020)
- Research Travel Grant, to present at GSA, UC Davis — $500 (2019)
- Everett Dale Jackson Memorial Scholarship, UC Davis — $5,000 (2018)
- Michael Alan Anthony Grant, UC Davis — $5,000 (2018)

### Publications

Author name in **bold**.

1. **Rees, H.**, Crompton, O., Lourenço, M., WinklerPrins, L., Larsen, L., Caylor, K.K., &
   Ganti, V. *(in prep).* A 41-year monthly inundation record reveals abrupt redistribution
   of flood waters across the Okavango Delta since 2012. *To be submitted to Nature
   Communications.*
2. WinklerPrins, L., Yang, W., Crompton, O., Lourenço, M., Ruer, M., Taylor, R.,
   **Rees, H.**, & Larsen, L. *(in prep).* A validated land cover classification of the
   Okavango Delta, 2023. *To be submitted to Remote Sensing of the Environment.*
3. **Rees, H.**, Ganti, V., Greenberg, E., Li, G., & Caylor, K. *(submitted).* Calibrating
   the geomorphic clock: remote sensing of sediment storage and transit timescales in
   alluvial river corridors. *Geophysical Research Letters.*
4. WinklerPrins, L., Fontenot, K., … **Rees, H.**, et al. *(submitted).* Early career
   perspectives on the next generation of deltaic science. *Estuaries and Coasts.*
5. McLeod, J.S., Ganti, V., Hampson, G.J., Bell, R.E., Slater, L.J., **Rees, H.**, &
   Whittaker, A.C. *(submitted).* Global sediment transport intermittency is set by river
   planform. *Nature Geoscience.*
6. **Rees, J.**, Lininger, K.B., Landis, J., & Briles, C. (2024). Assessing controls on
   sedimentation rates and sediment organic carbon accretion in beaver ponds. *Science of
   the Total Environment.* [DOI](https://doi.org/10.1016/j.scitotenv.2024.174951)
7. Ockelford, A., Wohl, E., … **Rees, J.**, … Aarnink, J. (2024). Working with wood in
   rivers in the Western United States. *River Research and Applications.*
8. Pinter, N., & **Rees, J.** (2021). Measuring success for managed flood retreat and
   community relocation in the Midwest USA. *Natural Hazards.*
   [DOI](https://doi.org/10.1007/s11069-021-04592-1)
9. Pinter, N., Ishiwatari, M., Nonoguchi, A., Tanaka, Y., Casagrande, D., Durden, S., &
   **Rees, J.** (2019). Large-scale managed retreat and structural protection following the
   2011 Japan tsunami. *Natural Hazards.*
   [DOI](https://doi.org/10.1007/s11069-019-03602-7)
10. **Rees, J.** (2019). Literature review of mechanisms and timescales involved in the
    termination of the African Humid Period. *Prized Writing, University of California, Davis.*

### Presentations & Talks

- **Rees, H.**, Ganti, V., Greenberg, E., Li, G., & Caylor, K. (Dec 2025). Calibrating the
  geomorphic clock: remote sensing of sediment storage and transit timescales in alluvial
  river corridors. *American Geophysical Union Fall Meeting,* New Orleans, LA.
- Triantafillou, S., **Rees, H.**, et al. (Dec 2025). International approaches to freshwater
  ecosystems: the Noce River, Italy and Yuba River, California, USA. *American Geophysical
  Union Fall Meeting,* New Orleans, LA.
- Scamardo, J., Lininger, K.B., & **Rees, H.** (Dec 2025). Synthesizing drivers of beaver dam
  sediment storage across headwater systems. *American Geophysical Union Fall Meeting,*
  New Orleans, LA.
- Scamardo, J., Lininger, K.B., & **Rees, H.** (Oct 2025). How much sediment can a beaver dam
  store: synthesizing drivers of beaver pond sedimentation and predicting sediment storage
  benefits of beaver dams. *7th International Symposium on River Science,* Davis, CA.
- **Rees, H.**, Ganti, V., Greenberg, E., Li, G., & Caylor, K. (Apr 2025). Calibrating the
  geomorphic clock: remote sensing of sediment storage and transit timescales in alluvial
  river corridors. *Southern California Geomorphology Symposium,* Los Angeles, CA.
- **Rees, J.**, Ganti, V., & Caylor, K. (Dec 2024). Natural and anthropogenic controls on
  floodplain reworking timescales and transient sediment storage in lowland river corridors.
  *American Geophysical Union Fall Meeting,* Washington, D.C.
- Lininger, K.B., & **Rees, J.** (Oct 2024). Sedimentation & carbon accretion in beaver
  ponds & implications for restoration. *BeaverCon,* Boulder, CO.
- **Rees, J.**, Lininger, K.B., Briles, C., & Landis, J.D. (Mar 2023). Beaver dams as sites
  of carbon accretion and sediment storage and implications for river restoration.
  *Association of American Geographers Annual Meeting,* Denver, CO.
- **Rees, J.**, Lininger, K.B., Briles, C., & Landis, J.D. (Dec 2022). Quantifying sediment
  and carbon storage and aggradation rates within beaver-mediated river corridors in the
  Southern Rocky Mountains, USA. *American Geophysical Union Fall Meeting,* Chicago, IL.
- **Rees, J.** (Apr 2022). Assessing managed flood retreat and community relocation as a
  flood mitigation and climate adaptation measure in small communities. *CU Annual Hydrologic
  Sciences Symposium,* Boulder, CO.
- Ishiwatari, M., Pinter, N., Casagrande, D., & **Rees, J.** (Aug 2021). Relocation programs:
  making resilient communities through involvement of diversified groups in Japan and the US.
  *8th International Conference on Flood Management,* Iowa City, IA.
- **Rees, J.**, & Pinter, N. (May 2019). The effectiveness of managed retreat and community
  relocation as a flood mitigation measure in small Midwest communities. *GSA Cordilleran
  Section Annual Meeting,* Portland, OR.
- **Rees, J.** (Feb 2019). Field data collection with QField. *#MaptimeDavis, UC Davis Center
  for Spatial Sciences,* Davis, CA.

### Teaching Experience

- Landscapes and Water — Lead Teaching Assistant (2 semesters)
- Introduction to Geographic Information Systems — Teaching Assistant (2 quarters)
- Global Warming — Teaching Assistant (1 quarter)
- Quantitative Geomorphology — Lead Teaching Assistant (2 quarters)

### Field Experience

- **Sierra Nevada, California** (2025, 3 weeks) — geomorphic/hydrologic surveys and sediment
  coring in beaver meadows
- **Dolomites, Italy** (2025, 2 weeks) — international approaches to freshwater management
- **Antioquia, Colombia** (2023, 2 weeks) — field training and hydrologic surveys on the
  Magdalena River
- **Rocky Mountains, Colorado** (2022, 6 weeks) — geomorphic/hydrologic surveys and sediment
  coring in natural beaver meadows
- **Cascade Mountains, California** (2019, 6 weeks) — aerial (drone) and geomorphic/hydrologic
  surveys to monitor beaver meadow restoration
- **Midwest USA** (2019, 2 weeks) — structure surveys and stakeholder interviews for managed
  retreat research
- **Northern Japan** (2018, 1 week) — coastal flood mitigation measures following the 2011
  Tohoku tsunami

### Skills

- **Programming languages:** Python (geopandas, shapely, rasterio, geemap, scikit-learn,
  whitebox, gdal, scipy), R (lidR), JavaScript
- **Software & toolkits:** Google Earth Engine Python API, ArcGIS Pro, QGIS, GitHub, HEC-RAS,
  HEC-LifeSim, HEC-FIA, Hazus, ENVI, Microsoft Office Suite, Affinity Designer, Adobe
- **Sensors & datasets:** Landsat, Sentinel 1/2, PlanetScope, NAIP, EMIT, SWOT, 3DEP, various
  LiDAR/DEMs, HydroSHEDS suite, GRWL, NASADEM, NLCD, Pekel et al. GSW, Dynamic Surface Water
  Extent (DSWE)

### Volunteer & Service

- Course Designer & Instructor, School for Scientific Thought (2026)
- Reviewer, *Communications Earth and Environment* (2025)
- Mentor, GEMSS Graduate Mentoring Program (2023–2025)
- Reviewer, *Water Resources Research* (2023)
- Cofounder, Fossil Free CU Divestment Campaign (2022–2023)
- Mentor, CU Boulder Graduate Mentoring Program (2022–2023)
- Course Designer & Instructor, Earth Explorers Science Mentoring Program (2022)
- Volunteer, 350 Sacramento (2021)
- Volunteer, California State Senate Campaign (2016)

### Membership

American Geophysical Union · Association of American Geographers · American Water Resources
Association · Phi Beta Kappa Honors Society

---

## Contact
{: #contact}

I'm always glad to talk rivers, beaver, remote sensing, or potential collaborations.

- **Email:** [jamesrees@ucsb.edu](mailto:jamesrees@ucsb.edu)
- **GitHub:** [github.com/huck-rees](https://github.com/huck-rees)
- **LinkedIn:** [huck-rees](https://www.linkedin.com/in/huck-rees-12b205157/)
- **Department:** Department of Geography, University of California, Santa Barbara
